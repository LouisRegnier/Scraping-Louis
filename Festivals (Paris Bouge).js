FestivalsParis = new Mongo.Collection('festivalsParis');


if (Meteor.isClient) {

  Meteor.call('getFestival', function (error, result1, result2) {
    if (error) {
  ...
  Template.festivalsParis.helpers({
    festivalsParis : function () {
      return FestivalsParis.find();
    }
  });
}
  ...
  Meteor.startup(function () {
    ...
    Meteor.methods({    
      getFestival: function () {
        ...
        result1 = Meteor.http.get("https://www.parisbouge.com/search?type=event&category=festival-musical&date_start="+annee+"-"+mois+"-"+jour+"&date_end="+annee+"-"+mois+"-"+jour+"&page=1");
        ...
        FestivalsParis.remove({});

        for (var i = 1; i<= nombrePages; i++) {
          result2 = Meteor.http.get("https://www.parisbouge.com/search?type=event&category=festival-musical&date_start="+annee+"-"+mois+"-"+jour+"&date_end="+annee+"-"+mois+"-"+jour+"&page="+i);
          ...
          $('div > div > h4 > a').each(function(j, elem) {
            nomFestival[j] = $(this).text().toUpperCase();
            ...
            var row = {};
            row["nomFestival"] = nomFestival [j];
            row["dates"] = dates [j];
            row["lieu"] = lieu [j];
            row["genre"] = genre [j];
            row["image"] = image [j];
            FestivalsParis.insert(row);
          });
        }
